...
def main() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    cwd = Path.cwd()
    output_path = resolve_user_path(args.output, base_dir, cwd)
    ...
    if args.mode in {"fixed-points", "both"}:
        fixpoint_header, fixpoint_rows = parse_biolqm_table(resolve_user_path(args.fixpoints, base_dir, cwd))
        if not args.no_dedup:
            fixpoint_rows = dedup_rows(fixpoint_rows)
    if args.mode in {"trap-spaces", "both"}:
        trap_header, trap_rows = parse_biolqm_table(resolve_user_path(args.trapspaces, base_dir, cwd))
        if not args.no_dedup:
