...
def main() -> None:
    ...
    if args.biolqm_jar:
        args.biolqm_jar = str(resolve_config_value_path(args.biolqm_jar, config_path, base_dir, cwd))
    else:
        maybe_biolqm_cmd = Path(str(args.biolqm_cmd))
        if maybe_biolqm_cmd.is_absolute() or any(sep in str(args.biolqm_cmd) for sep in ("\\", "/")):
            args.biolqm_cmd = str(resolve_config_value_path(str(args.biolqm_cmd), config_path, base_dir, cwd))
    fixpoints_output = resolve_config_value_path(args.fixpoints_output, config_path, base_dir, cwd) if args.fixpoints_output else None
    trapspaces_output = resolve_config_value_path(args.trapspaces_output, config_path, base_dir, cwd) if args.trapspaces_output else None
