...
def main() -> None:
    ...
    rules = parse_bnet(resolve_config_value_path(bnet_path, config_path, base_dir, cwd))
    ...
    out_path = resolve_config_value_path(output_path_value, config_path, base_dir, cwd)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    if canalization_output_value:
        canalization_output_path = resolve_config_value_path(canalization_output_value, config_path, base_dir, cwd)
    ...
    if essentiality_output_value:
        essentiality_output_path = resolve_config_value_path(essentiality_output_value, config_path, base_dir, cwd)
    ...
    if canalization_output_value:
        print(f"Canalization output: {resolve_config_value_path(canalization_output_value, config_path, base_dir, cwd)}")
    if essentiality_output_value:
        print(f"Essentiality output: {resolve_config_value_path(essentiality_output_value, config_path, base_dir, cwd)}")
